...
const oldButtonStart = `                      {/* Pestañas de severidad */}
                      <Button
...
const newButtonStart = `                      {/* Filtros Flotantes */}
...
pageContent = pageContent.replace(oldButtonStart, newButtonStart);

// Now, remove the old Drawer at the bottom
const regex = /\{\/\* Drawer de Filtros Avanzados \*\/\}\s*<Drawer[\s\S]*?<\/Drawer>/g;
pageContent = pageContent.replace(regex, '');

fs.writeFileSync(pagePath, pageContent, 'utf8');
console.log("Floating filter window refactored");
